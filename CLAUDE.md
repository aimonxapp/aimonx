...
- ⛔ **`main` è il sito pubblicato, ed è misurato, non più un'ipotesi:** un merge su `main` **è una pubblicazione al mondo**, perché Pages costruisce dalla radice di `main`. La regola di ramo qui pesa il doppio. ⭐ **Esercitato cinque volte — W3, W7, W8, W9, W10** — ogni volta con l'autorizzazione esplicita di Pier: `main` è sempre avanzato in **fast-forward**, mai un `--force`, mai un ramo cancellato. ⭐ **E ogni volta la build di Pages è risultata IDENTICA AL BYTE a quella locale**, file per file — nel W10, **20 file su 20**. ⚠️ **I tempi di ricostruzione NON si scrivono qui:** cambiano a ogni merge (nel W9, 149,6 s) e si leggono con `gh api repos/aimonxapp/aimonx/pages/builds/latest`.
