...
- **Cosa pubblica oggi:** ⭐ **la landing**, online dal giro W7 — titolo *«AIMONX — the technical logbook for sport shooters»*, il foglio di stile è il nostro, il font scritto a mano è servito da noi, e le richieste verso l'esterno sono **zero, misurate sul sito vero**. ⛔ **È l'unica pagina ONLINE:** Privacy Policy (`/privacy-policy/`), Support (`/support/`) e Terms (`/terms/`) sono **costruite dal giro W9 e ferme su ramo** (`WA1`, `WA2`, `WA3`) — esistono nel repo, non nel mondo, e ⛔ **fra le due cose c'è un merge che autorizza Pier.** *(`curl -sS https://aimonx.app/ | grep -E 'title|generator'`.)*
...
- ⛔ **`main` è il sito pubblicato, ed è misurato, non più un'ipotesi:** un merge su `main` **è una pubblicazione al mondo**, perché Pages costruisce dalla radice di `main`. La regola di ramo qui pesa il doppio. ⭐ **Esercitato due volte, nei giri W3 e W7**, ogni volta con l'autorizzazione esplicita di Pier: `main` è avanzato in fast-forward e Pages ha ricostruito in 42 s e in 36 s. ⭐ **E ogni volta la build di Pages è risultata IDENTICA AL BYTE a quella locale**, file per file.
...
node scripts/contrasto.mjs http://127.0.0.1:4000/{,privacy-policy/,support/,terms/}
node scripts/sbordi.mjs    http://127.0.0.1:4000/{,privacy-policy/,support/,terms/}
