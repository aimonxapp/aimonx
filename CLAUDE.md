...
| **come si guarda l'anteprima con un browser vero** | `scripts/anteprima-playwright.mjs` — due screenshot, gli errori in console, e ⛔ **ogni richiesta verso un dominio esterno** · `scripts/contrasto.mjs` — il contrasto di **ogni** testo contro WCAG 2.1 AA, preso dai colori calcolati dal browser e ⛔ **non da una lista scritta a mano**, che diverge dal CSS il giorno dopo. ⚠️ **Non sono test che passano o falliscono: sono misure**, e il giudizio è di chi legge |
| **se il testo in pagina è ancora quello approvato da Pier** | `scripts/testo-approvato.sh` — cerca ogni stringa di `_data/landing.yml` **alla lettera** nella bozza di Cowork. ⛔ **Non dice se un testo è buono**, dice se è *quello*; e ⚠️ **scatta anche quando cambia la bozza** — lì non c'è un errore da riparare, c'è una consegna nuova da portare nel repo |
...
- **Cosa pubblica oggi:** ⭐ **la landing**, online dal giro W7 — titolo *«AIMONX — the technical logbook for sport shooters»*, il foglio di stile è il nostro, il font scritto a mano è servito da noi, e le richieste verso l'esterno sono **zero, misurate sul sito vero**. ⛔ **È l'unica pagina che esiste:** Privacy Policy, Support e Terms non ci sono ancora (`WA1`, `WA2`, `WA3`). *(`curl -sS https://aimonx.app/ | grep -E 'title|generator'`.)*
