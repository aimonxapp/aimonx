...
- **Pages pubblica dal ramo `main`, cartella radice `/`**, con **Jekyll** (`"build_type": "legacy"`); dominio `aimonx.app`, certificato approvato. ⛔ **`https_enforced` è `false`**: `http://aimonx.app` risponde 200 e non rimanda a HTTPS, e la pagina dichiara `<link rel="canonical" href="http://aimonx.app/">`. *(`gh api repos/aimonxapp/aimonx/pages`, `curl -sSI http://aimonx.app/`.)*
